...
unlink(x = "./*.RData")
save.image("trimming_done.RData")
...
unlink(x = "./*.RData")
save.image("./Counting_done.RData")
...
save.image(
  paste0(
    str_replace_all(string = Sys.Date(), pattern = "-", replacement = "_"), 
    "_COVID19_61samples.RData"
  )
)
writeLines(
  text = capture.output(sessionInfo()), 
  con = paste0(format(Sys.time(), "%y_%m_%d"), "_COVID19_61samples.txt")
)
